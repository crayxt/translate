...
tasks.revise_translations.run_from_args
        ...
        +--> core.review_bundle.load_review_bundle
...
2. `core.review_bundle.load_review_bundle()` picks the right bundle builder for the file format.
...
5. `run_from_args()` applies `--probe` through the shared `limit_items()` helper.
...
9. `run_revision()` prepares each batch with `build_revision_request_contents()`, which calls `build_review_message_payload()`, `build_revision_request_payload()`, and `build_task_request_contents()`.
10. `run_review_batches()` performs the API calls through `run_model_batches()`, retries missing item ids through `build_retry_contents()`, and parses responses with `parse_revision_response()`.
11. `parse_revision_response()` funnels through `_normalize_revision_payload()` and `normalize_task_issue()` so revision actions and issues are typed consistently.
12. `on_batch_completed()` applies accepted updates with `apply_revision_to_item()`. For plurals, that path also uses `build_candidate_plural_forms()`.
13. After all batches, `run_from_args()` prints summary samples with `print_change_samples()` and `print_issue_samples()`.
14. If `--dry-run` is not set, the task writes the generated output via the bundle `save_callback()` and then finalizes the destination path with `move_output_file()`.
